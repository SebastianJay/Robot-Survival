...
def rand_event():
    global robots, oil, metal, expand_chance, improved_collect, improved_expand, expansion, fortification
    rand = random.random()
...
def end_turn():
    global robots, oil, metal, expand_chance, improved_collect, improved_expand, improved_upkeep, expansion, fortification
    if improved_upkeep:
        oil -= math.ceil((robots + fortification) * 0.5)
        print("Lost " + str(math.ceil((robots + fortification) * 0.5)) + " oil for upkeep!")
    else:
        oil -= math.ceil(robots + fortification * 0.75)
        print("Lost " + str(math.ceil((robots + fortification) * 0.75)) + " oil for upkeep!")
    rand_event()
...
def collect():
    ...
    choice = int(input("\n"))
    if choice == 1:
        choice = int(input("How many robots to spend: "))
        if improved_collect:
            oil += 20 * choice
            robots -= choice
            print("Gained " + str(20 * choice) + " oil; lost " + str(choice) + " robots!")
        else:
            oil += 10 * choice
            robots -= choice
            print("Gained " + str(10 * choice) + " oil; lost " + str(choice) + " robots!")
    elif choice == 2:
        if improved_collect:
            metal += 20
            robots -= 1
...
def printerface():
    ...
    print("4. Fortify (lose a robot to gain defense")
